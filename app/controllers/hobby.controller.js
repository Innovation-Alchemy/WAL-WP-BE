...
exports.getAllHobbies = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error retrieving hobbies", error: error.message });
    }
  };
  ...
  exports.getHobbyById = async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({ message: "Hobby retrieved successfully", data: hobby });
    } catch (error) {
      console.error("Error retrieving hobby:", error);
      res.status(500).json({ message: "Error retrieving hobby", error: error.message });
    }
  };
  ...
  exports.addHobby = async (req, res) => {
    try {
      const { name } = req.body;
      const newHobby = await db.Hobby.create({ name });
      res.status(201).json({ success: true, data: newHobby });
    } catch (error) {
      const status =
        error.name === "SequelizeUniqueConstraintError" ? 409 : 400;
      res.status(status).json({ message: "Error adding hobby", error: error.message });
    }
  };
...
exports.assignHobbiesToUser = async (req, res) => {
    ...
    try {
      // Validate hobby IDs
      if (!Array.isArray(hobbyIds) || !hobbyIds.every((hid) => Number.isInteger(hid))) {
        return res.status(400).json({ message: "Hobby IDs must be an array of integers" });
      }
      ...
      if (invalidIds.length > 0) {
        return res.status(400).json({ message: "Some hobby IDs do not exist", invalidIds });
      }
      ...
      res.status(200).json({ message: "Hobbies assigned successfully", data: updatedHobbies });
    } catch (error) {
      console.error("Error assigning hobbies:", error);
      res.status(500).json({ message: "Error assigning hobbies", error: error.message });
    }
  };
  ...
  exports.removeHobbiesFromUser = async (req, res) => {
    ...
    try {
      ...
      const invalidHobbyIds = hobbyIds.filter((id) => !validHobbyIds.includes(id));
      ...
      const hobbiesToRemove = hobbyIds.filter((hid) => currentHobbies.includes(hid));
      const notFoundHobbies = hobbyIds.filter((hid) => !currentHobbies.includes(hid));
  
      // 6. Remove the hobbies found in the user's current hobbies
      let updatedHobbies = currentHobbies;
      if (hobbiesToRemove.length > 0) {
        updatedHobbies = currentHobbies.filter((hid) => !hobbiesToRemove.includes(hid));
        user.hobbies = updatedHobbies;
        await user.save();
      }
      ...
      if (hobbiesToRemove.length > 0 && notFoundHobbies.length > 0) {
        ...
        message = "No hobbies were removed because none of the provided hobby IDs were assigned to the user.";
      }
      ...
      res.status(500).json({ message: "Error removing hobbies", error: error.message });
    }
  };
  ...
  exports.updateHobby = async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({ message: "Hobby updated successfully", data: hobby });
    } catch (error) {
      console.error("Error updating hobby:", error);
      res.status(500).json({ message: "Error updating hobby", error: error.message });
    }
  };
  ...
  exports.deleteHobby = async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ message: "Error deleting hobby", error: error.message });
    }
  };
